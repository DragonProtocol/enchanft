...
export default function MyMint() {
  ...
  console.log({ belongToMe })
  return (
    <div>
      <h2>
        Info: {params.mint} 【BelongToMe：{belongToMe}】
      </h2>
      {params.mint && <NFTShower mint={params.mint} uri={metadata.data?.uri || ''} />}
      {belongToMe ? (
        <>
          <button onClick={nftCopy}>create</button>
          <button onClick={injectSol}>InjectSOL</button>
          <button onClick={injectNFT}>injectNFT</button>
          <button onClick={extractSol}>extractSol</button>
          <button onClick={extractNFT}>extractNFT</button>
