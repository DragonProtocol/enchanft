...
export function Feed({
  ...
  function RefreshButton() {
    if (refreshing) {
      return (
        <button className="btn black rounded-border">
          <img src="/img/icons/loading-white.svg" height="15" />
        </button>
      );
    } else {
      return (
        <button className="btn black rounded-border" onClick={() => refresh()}>
          <img
            src="/img/icons/refresh-white.png"
            height="11"
            className="mright-5"
          />
          <span>Refresh</span>
  ...
  function AlgoPicker() {
    ...
    return (
      <div ref={wrapperRef}>
        <button
          className="btn black rounded-border v-align-items-center"
          onClick={() => setShowAlgos(true)}
        >
          <span className="mright-5">
            <CurrentAlgo />
          </span>
          <img src="/img/icons/caret-down-white.png" height="5" />
        </button>
        {showAlgos && (
          <div className="floating-menu">
            <p onClick={() => updateAlgo('recommendations')}>
              {algo == 'recommendations' && (
                <img
                  src="/img/icons/check-green.png"
                  className="mright-5"
                  height="14"
                />
              )}
            ...
            <p onClick={() => updateAlgo('all-posts')}>
              {algo == 'all-posts' && (
                <img
                  src="/img/icons/check-green.png"
                  className="mright-5"
            ...
            <p onClick={() => updateAlgo('all-posts-non-filtered')}>
              {algo == 'all-posts-non-filtered' && (
                <img
                  src="/img/icons/check-green.png"
                  className="mright-5"
                  height="14"
  ...
  function LoadMoreButton() {
    if (refreshing) {
      ...
      return (
        <div className="p-15">
          <div className="btn black" onClick={() => loadMore()}>
            Load older posts
          </div>
        </div>
      );
          ...
          {loading ? (
            ...
            <>
              ...
              {!posts || posts.length == 0 ? (
                <p className="center w-100 tertiary">
                  There isn't any post shared here.
                </p>
              ) : (
                <>
                  <div className="flex-row mbottom-10 relative">
                    ...
                    {!context && !did && <AlgoPicker />}
                  </div>
                  <Posts posts={posts} context={context} type="feed" />

                  {posts.length >= (page + 1) * 50 - 1 && <LoadMoreButton />}
                </>
              )}
            </>
          )}
        ...
        <>
          ...
          {!posts || posts.length == 0 ? (
            <div className="flex-1">
              <p className="center w-100 tertiary">
                There isn't any post shared in this channel.
              </p>
            </div>
          ) : (
