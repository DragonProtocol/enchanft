  ...
  function GroupPfp({ onClick }) {
    if (group.content.pfp || group.content.metadata?.pfp) {
      if (group.content.pfp) {
        return (
          <img
            src={group.content.pfp}
            className={isAdmin ? 'pfp admin' : 'pfp'}
      ...
      if (group.content.metadata?.pfp) {
        return (
          <img
            src={group.content.metadata?.pfp}
            className={isAdmin ? 'pfp admin' : 'pfp'}
            onClick={onClick ? onClick : null}
          />
        );
            ...
            {group && group.content && (
              <>
                {isAdmin ? (
                  <GroupPfp
                    onClick={() =>
                      setModalVis('update-group', true, group, setGroup)
                    }
                  />
                ) : (
                  <GroupPfp />
              ...
              {isAdmin && (
                <div className="">
                  <div
                    className="btn white"
                    onClick={() =>
                      setModalVis('create-channel', true, group, setGroup)
                    }
                  >
                    + Add a channel
                  </div>
