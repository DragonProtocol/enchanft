...
var tunnelContainer = document.querySelector('.tunnel-animation');
var tunnelCanvas = document.createElement('canvas');
tunnelContainer.appendChild(tunnelCanvas);
var w = tunnelContainer.offsetWidth,
  h = tunnelContainer.offsetHeight;
tunnelCanvas.style.width = w + 'px';
tunnelCanvas.style.height = h + 'px';
window.onresize = function () {
  tunnelContainer = document.querySelector('.tunnel-animation');
  (w = tunnelContainer.offsetWidth), (h = tunnelContainer.offsetHeight);
  tunnelCanvas.style.width = w + 'px';
  tunnelCanvas.style.height = h + 'px';
  console.log({ tunnelContainer, w, h });
  camera.aspect = w / h;
...
var camera = new THREE.PerspectiveCamera(60, w / h, 0.001, 1000);
...
for (i = 0; i < geometry.faces.length; i++) {
  ...
  geometry.faces[i].color = new THREE.Color(`rgba(90,90,90,${0.5 / i})`);
}
...
function render() {
  ...
  var pt2 = path.getPointAt((pct + 0.01) % 1);
