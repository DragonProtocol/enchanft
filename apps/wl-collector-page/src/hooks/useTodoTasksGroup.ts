...
function useTodoTasksGroup() {
  ...
  const todoItems = useMemo(
    () =>
      todoTasks.filter((task) => task.status === TaskTodoCompleteStatus.TODO),
    [todoTasks]
  );
  const inProgressItems = useMemo(
    () =>
      todoTasks.filter(
        (task) => task.status === TaskTodoCompleteStatus.IN_PRGRESS
      ),
    [todoTasks]
  );
  const completedItems = useMemo(
    () =>
      todoTasks.filter(
        (task) => task.status === TaskTodoCompleteStatus.COMPLETED
      ),
    [todoTasks]
  );
  const wonItems = useMemo(
    () =>
      todoTasks.filter((task) => task.status === TaskTodoCompleteStatus.WON),
    [todoTasks]
  );
  const lostItems = useMemo(
    () =>
      todoTasks.filter((task) => task.status === TaskTodoCompleteStatus.LOST),
    [todoTasks]
  );
