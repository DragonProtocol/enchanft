...
import {
  getInjectSOLFromMints,
  getMetadataFormMints,
  getValidNFTokensWithOwner,
} from '../../utils';
...
export const getMyNFTokens = createAsyncThunk(
  ...
  async (
    ...
    thunkAPI.dispatch(
      getMyNFTData({ nfts: filteredTokens, connection, synftContract })
    );
  ...
  async (
    {
      ...
      connection,
      synftContract,
    }: { nfts: Token[]; connection: Connection; synftContract: SynftContract },
    thunkAPI
  ) => {
