...
import TaskActionList, {
  TaskActionItemsType,
} from '../components/business/task/TaskActionList';
import {
  ActionType,
  TaskAcceptedStatus,
...
} from '../components/business/task/TaskDetailContent';
import {
...
const Task: React.FC = () => {
  ...
  const dispatchFetchTaskDetail = useCallback(
    () => id && dispatch(fetchTaskDetail(Number(id))),
    [id]
  );
  ...
  const { accountOperationType, accountOperationDesc, handleAccountOperation } =
    useAccountOperationForChain(data?.project?.chainId);
  ...
  // handles: take, verify
  const { takeTask: takeTaskState } = useAppSelector(
    selectUserTaskHandlesState
  );
  ...
  const userFollowedCommunityIds = useAppSelector(
    selectIdsByUserFollowedCommunity
  ).map((item) => Number(item));

  // verify task queue
  const verifingTaskIds = useAppSelector(selectIdsVerifyTaskQueue).map((item) =>
    Number(item)
  );
  // verify action queue
  const verifingActionIds = useAppSelector(selectIdsVerifyActionQueue).map(
    (item) => Number(item)
  );
  ...
  const allowHandleAction =
    data?.acceptedStatus === TaskAcceptedStatus.DONE &&
    data?.status !== TaskTodoCompleteStatus.CLOSED;

  // verify action
  const displayVerify =
    allowHandleAction &&
    actionItems.some((v) => v.status === UserActionStatus.TODO);
  ...
  if (loadingVerify) {
    verifyingActions = actionItems
      .filter((item) => item.status === UserActionStatus.TODO)
      .map((item) => item.id);
  }
  ...
  const displayParticipants =
    ...
    participants.takers >= TASK_PARTICIPANTS_DISPLAY_MIN_NUM;
  return (
    <TaskDetailWrapper>
      <TaskDetailBodyBox>
        {data?.status === TaskTodoCompleteStatus.CLOSED && (
        ...
        <TaskDetailBodyMainBox>
          <TaskDetailHeaderBox>
            ...
            <ShareButton
              onClick={() =>
                tweetShare(SHARE_EVENT_TWEET_CONTENTS, taskShareUrl)
              }
            >
              <IconShare size="16px" />
            </ShareButton>
            ...
            {isDesktop &&
              project &&
              checkProjectAllowed(Number(project.id)) &&
              isCreator && (
                <ManageButton
                  onClick={() => toWlModPageTaskDetail(data.id, project.slug)}
                >
                  Tasks Management
                </ManageButton>
              )}
                ...
                <>
                  ...
                  <TaskListBox>
                    ...
                    <TaskActionList
                      ...
                      onFollowCommunity={(action) =>
                        handleFollowCommunity(action.communityId)
                      }
                      ...
                      onVerifyAction={(action) =>
                        dispatch(verifyAction(action))
                      }
                      onCustomAction={(action) =>
                        dispatch(completionAction(action))
                      }
