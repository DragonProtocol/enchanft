...
type ExploreRecommendTasksState = EntityState<ExploreRecommendTaskItemEntity> & {
...
// 列表信息数据范式化
export const exploreRecommendTasksEntity = createEntityAdapter<ExploreRecommendTaskItemEntity>({
  selectId: (item) => item.id,
})

// 初始化列表信息
const tasksState: ExploreRecommendTasksState = exploreRecommendTasksEntity.getInitialState({
...
export const selectExploreRecommendTasksState = (state: RootState) => state.exploreRecommendTasks
export const { selectAll } = exploreRecommendTasksEntity.getSelectors((state: RootState) => state.exploreRecommendTasks)
