...
export const isExternalLinkRiskWarningUrl =
  location.href.startsWith(ExternalLinkRiskWarningUri + '?') && getExternalLink()
...
export const startExternalLinkNavigationListener = () => {
  ...
  document.getElementById('root')?.addEventListener('click', (e) => {
    ...
    if (target.nodeName === 'A') {
      ...
      if (isHyperlink(href) && !noWarningDomains.some((domain) => hostname.endsWith(domain))) {
        e.preventDefault()
  ...
  return (
    <>
      ...
      <MobileRedirectWrapper>
        ...
        <RedirectDescBox>
          <span>⚠️ Please make sure your account is safe when accessing external websites. </span>
          <span>⚠️ Do not give out your wallet information to others.</span>
          <span>
            ⚠️ When you find that the site is not secure, please
            <RedirectDescBold> do not approve the transaction and disconnect the wallet.</RedirectDescBold>
          </span>
          <UnderlineText>
            Make sure you read the above information carefully before accessing other sites.
          </UnderlineText>
        </RedirectDescBox>
        <ContinueBtn onClick={() => (location.href = externalLink)}>Got it. Continue →</ContinueBtn>
      </MobileRedirectWrapper>
    </>
  )
}
