/*
 * @Author: shixuewen friendlysxw@163.com
 * @Date: 2022-07-01 18:28:12
 * @LastEditors: shixuewen friendlysxw@163.com
 * @LastEditTime: 2022-07-19 14:44:13
 * @Description: enchanfted item
 */

import React from 'react'
import styled from 'styled-components'
import { NFTDataItem } from '../../../types/synft'
import SolanaIcon from './icons/solana.png'
import { lamportsToSol } from '../../../utils/solana'
import { MEDIA_BREAK_POINTS, VIEW_LAMPORTS_DECIMAL } from '../../../constants'
export type EnchanftedItemDataType = NFTDataItem
export type EnchanftedItemViewConfigType = {}

export type EnchanftedItemDataViewType = {
  data: EnchanftedItemDataType
  viewConfig?: EnchanftedItemViewConfigType
}

export type EnchanftedItemHandlesType = {}

export type EnchanftedItemProps = EnchanftedItemDataViewType & EnchanftedItemHandlesType

const defaultViewConfig: EnchanftedItemViewConfigType = {}

const EnchanftedItem: React.FC<EnchanftedItemProps> = ({ data, viewConfig }: EnchanftedItemProps) => {
  const { externalMetadata, injectSolAmount = 0, mint } = data
  return (
    <NFTCardWrapper>
      <div className="img-box">
        {/* {data.hasCopied && <span className="tag tag-synthesized">Synthesized</span>}
        {data.hasInjected && <span className="tag tag-enchanted">Enchanted</span>} */}
        <img alt="" src={data.image} className="img" />
      </div>
      <EnchaNFTedNameBox>
        <EnchaNFTedName>{data.name}</EnchaNFTedName>
        <EnchaNFTedProjectName>
          {externalMetadata?.collection?.name || externalMetadata?.collection?.family || 'unknown collection'}
        </EnchaNFTedProjectName>
      </EnchaNFTedNameBox>
      <EnchaNFTedAmountBox>
        <EnchaNFTedAmountTitle>EnchaNFTed</EnchaNFTedAmountTitle>
        <EnchaNFTedAmount>
          <img src={SolanaIcon} alt="" />
          <span>{lamportsToSol(injectSolAmount).toFixed(VIEW_LAMPORTS_DECIMAL)}SOL</span>
        </EnchaNFTedAmount>
      </EnchaNFTedAmountBox>
    </NFTCardWrapper>
  )
}
export default EnchanftedItem
const NFTCardWrapper = styled.div`
  width: 100%;
  height: 100%;
  border: 2px solid #222222;
  box-sizing: border-box;
  background: #fff;
  box-shadow: 0px 4px 0px rgba(0, 0, 0, 0.25);
  .img-box {
    width: 100%;
    height: 250px;
    box-sizing: border-box;
    position: relative;
    @media (max-width: ${MEDIA_BREAK_POINTS.sm}px) {
      height: auto;
    }
    .tag {
      position: absolute;
      top: 0;
      left: 0;
      padding: 8px;
      background: #e4ffdb;
      border: 2px solid #222222;
      border-top: none;
      border-left: none;
      box-sizing: border-box;
      font-size: 12px;
    }
    .tag-synthesized {
      background: #fffbdb;
    }
    .tag-enchanted {
      background: #e4ffdb;
    }
    .img {
      width: 100%;
      height: 100%;
    }
  }
  .name {
    font-size: 14px;
    line-height: 14px;
    color: #222222;
  }
`
const EnchaNFTedNameBox = styled.div`
  padding: 16px;
`
const EnchaNFTedName = styled.div`
  font-size: 16px;
  line-height: 16px;
`

const EnchaNFTedProjectName = styled.div`
  font-size: 12px;
  line-height: 12px;
  color: #3dd606;
  margin-top: 12px;
`

const EnchaNFTedAmountBox = styled.div`
  padding: 28px 16px;
  display: flex;
  justify-content: space-between;
  align-items: center;
  background: rgba(34, 34, 34, 0.05);
`

const EnchaNFTedAmountTitle = styled.div`
  font-size: 12px;
  line-height: 12px;
`
const EnchaNFTedAmount = styled.div`
  display: flex;
  gap: 12px;
  align-items: center;
  font-size: 14px;
  line-height: 14px;
  img {
    width: 24px;
    height: 24px;
  }
`
