...
const TaskActionItem: React.FC<TaskActionItemProps> = ({
  ...
  const renderAction = () => {
    switch (type) {
      ...
      case ActionType.MEET_CONTRIBUTION_SCORE:
        // 达成贡献度
        return (
          <ActionContributionScore data={data} allowHandle={allowHandle} />
        );
      ...
      case ActionType.ANSWER_VERIFY:
        ...
        return (
          <ActionVerifyAnswer
            data={data}
            allowHandle={allowHandle}
            onQuestionVerifyConfirm={onQuestionVerifyConfirm}
          />
        );
      case ActionType.QUESTIONNAIRE:
        ...
        return (
          <ActionQuestionnaire
            data={data}
            allowHandle={allowHandle}
            onQuestionConfirm={onQuestionConfirm}
          />
        );
  ...
  const renderWeight = useCallback(() => {
    if (dispalyLuckyDrawWeight && actionData.lucky_draw_weight) {
      return (
        <TaskActionLuckyDrawWeight>
          +{actionData.lucky_draw_weight}
        </TaskActionLuckyDrawWeight>
      );
    }
    return null;
