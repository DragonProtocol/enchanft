...
function Home() {
  ...
  const exploreDataCache = useRef<{
    initRequsetAfter: boolean;
    pageIndex: number;
    data: NftDataItem[];
  }>({
  ...
  const { ref: nftListScrollRef, inView } = useInView({
    root: getLayoutMainScrollBox(),
    threshold: 0,
  });
  // 监听列表数据的请求状态
  useEffect(() => {
    ...
    if (exploreNFTStatus === 'init' && collections.length > 0) {
      ...
      if (exploreDataCache.current.initRequsetAfter) {
        ...
        if (exploreDataCache.current.pageIndex < collections.length - 1) {
          ...
          dispatch(
            getExploreDataWithCollectionId({
              collectionId: collections[exploreDataCache.current.pageIndex + 1],
              connection,
            })
          );
        }
  ...
  useEffect(() => {
    ...
    if (
      tab === 'explore' &&
      showExplorePageIndex < exploreDataCache.current.pageIndex &&
      inView
    ) {
  ...
  switch (tab) {
    case 'explore':
      nftListLoading =
        ...
        !exploreDataCache.current.initRequsetAfter;
      break;
    case 'my':
      ...
      <div className="tab">
        <div className="guide-item guide-explore">
          <div className="guide-desc">
            🔥 View Popular NFTs and create synthetic NFTs
          </div>
          <ButtonWarning
            className="guide-btn"
            onClick={() => switchList('explore')}
          >
            {'> Explore NFT <'}
          </ButtonWarning>
        </div>
        <div className="guide-item guide-view-my">
          <div className="guide-desc">
            🔗 EMBED NFTs AND SOL INTO YOUR OWN NFTs
          </div>
        ...
        {tab === 'my' ? (
          <>
            <div className="list-title">
              <SplitTextOpacity ref={titleRefMy}>
                My collection
              </SplitTextOpacity>
            </div>
            <div className="list-desc">
              <SplitTextOpacity ref={titleRefMy2}>
                EnchaNFT your own NFTs
              </SplitTextOpacity>
            </div>
          </>
        ) : (
          <div className="list-title">
            <SplitTextOpacity ref={titleRefExplore}>
              Choose and Create Synthetic NFTs
            </SplitTextOpacity>
          </div>
        )}
        <div className="list">
